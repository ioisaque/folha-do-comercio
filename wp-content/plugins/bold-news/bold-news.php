...
class bt_section {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		if ( $subposts > 0 ) {
			...
			$posts = boldthemes_get_template_posts_data( $subposts, 0, $posts_category, $bt_local_exclude_ids, $posts_author, $show_sticky, 'blog' );
			...
			foreach ( $posts as $item ) {
				...
				$video_replace[] = $item['video'] != '' ? '<div class="btMediaBoxPopup"><div class="btMediaBoxPopupClose"></div>' . boldthemes_get_media_html( 'video', array( $item['video'] ) ) . '</div>' : '';
...
class bt_clients {
	...
	static function handle_shortcode( $atts, $content ) {
		...
		$output = '<div class="' . $el_class . ' btCarouselSmallNav boldClientList ' . $slides_padding .  $arrow_position . '"' . $style . '>';
			$output .= '<div class="bclPort" data-slides="' . $slides . '"'. $slick_data . '>';
				$output .= wptexturize( do_shortcode( $content ) );
			$output .= '</div>';
		$output .= '</div>';
...
function bt_map_sc() {
	if ( function_exists( 'bt_rc_map' ) ) {
		...
		bt_rc_map( 'bt_gmaps', array( 'name' => __( 'Google Maps', 'bt_plugin' ), 'description' => __( 'Google Maps with marker on specified coordinates', 'bt_plugin' ), 'container' => 'vertical', 'icon' => 'bt_bb_icon_bt_bb_gmaps_location', 'accept' => array( 'bt_header' => true, 'bt_button' => true, 'bt_counter' => true, 'bt_icon' => true, 'bt_image' => true, 'bt_text' => true, 'bt_hr' => true, 'bt_dropdown' => true, 'bt_service' => true ),
			'params' => array(
...
if ( ! class_exists( 'BT_Weather_Widget' ) ) {
	...
	class BT_Weather_Widget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			if ( $weather_data['code'] != false ) {
				if ( $this->type == 'now' ) {
					$new_title =  __( 'Now', 'bt_plugin' );
					if ( ! empty( $this->title ) ) {
						$new_title = $this->title;
					}

					echo '<span class="btIconWidget btWeatherWidget">';
						echo '<span class="btIconWidgetIcon">';
							echo boldthemes_get_icon_html( 'wi_' . $this->get_icon_code( $weather_data['code'] ), '', '', 'btIcoDefaultType btIcoAccentColor', '' );
						echo '</span>';
						echo '<span class="btIconWidgetContent">';
							echo '<span class="btIconWidgetTitle">' . $new_title . '</span>';
							echo '<span class="btIconWidgetText">' . $weather_data['temp'] . '&deg;' . $this->temp_unit . '</span>';
						echo '</span>';
					echo '</span>';
				} else if ( $this->type == 'today' ) {
					$new_title =   __( 'Today', 'bt_plugin' );
					if ( ! empty( $this->title ) ) {
						$new_title = $this->title;
					}
					...
					$new_title = __( 'Tomorrow', 'bt_plugin' );
					if ( ! empty( $this->title ) ) {
						$new_title = $this->title;
					}
